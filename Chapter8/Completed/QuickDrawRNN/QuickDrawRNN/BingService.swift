...
class BingService{
    ...
    let subscriptionKey = "<ADD YOUR SUBSCRIPTION KEY>"
    ...
    func syncSearch(searchTerm:String, count:Int=4) -> [BingServiceResult]{
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) { data, response, error in
            if error == nil, data != nil {
                ...
                print("Error using Bing image search service \(error?.localizedDescription ?? "")")
            }
            semaphore.signal()
        }
    ...
    */
    func syncDownloadImage(bingResult:BingServiceResult) -> CIImage?{
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            if error == nil, data != nil{
                let _ = response?.suggestedFilename ?? url.lastPathComponent
